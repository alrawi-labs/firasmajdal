...
interface Tooltip  { name: string; x: number; y: number; }
...
function featureToPath(f: Feature, b: Bounds): string {
  const g = f.geometry;
  if (g.type === "Polygon")
    return (g.coordinates as number[][][]).map((r) => ringToPath(r, b)).join(" ");
...
function getBounds(features: Feature[]): Bounds {
  ...
  features.forEach((f) => {
    ...
    if (f.geometry.type === "Polygon")
      (f.geometry.coordinates as number[][][]).forEach((r) => coords.push(...r));
    else
  ...
  return { minLon: minLon - pLon, maxLon: maxLon + pLon, minLat: minLat - pLat, maxLat: maxLat + pLat };
}
...
function Province({ d, isHovered, onMouseMove, onMouseEnter, onMouseLeave }: ProvinceProps) {
  return (
    <path
      d={d}
      ...
      style={{ cursor: "default", transition: "fill 0.2s ease, stroke 0.2s ease" }}
...
export function TurkeyMap() {
  ...
  useEffect(() => {
    fetch(GEOJSON_URL)
      ...
      .then((d: GeoJSON) => { setGeo(d); setLoading(false); })
  ...
  return (
    <>
      ...
      <section className="mt-16 mb-25" style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", userSelect: "none" }}>

        {/* ── Header ── */}
        <div className="mb-10" style={{ textAlign: "center", padding: "1.5rem 1rem 1rem" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "12px", marginBottom: "10px" }}>
            <span style={{ display: "block", height: "1px", width: "36px", background: `linear-gradient(90deg, transparent, ${GOLD})` }} />
            <span style={{ color: GOLD, fontFamily: "'Cinzel', serif", fontSize: "12px", fontWeight: 600, letterSpacing: "0.28em" }}>
              HİZMET KAPSAMIMIZ
            </span>
            <span style={{ display: "block", height: "1px", width: "36px", background: `linear-gradient(90deg, ${GOLD}, transparent)` }} />
          </div>
        ...
        >
          ...
          {loading && (
            <div style={{
              ...
              color: GOLD_DIM, fontFamily: "'Cormorant Garamond', serif", fontSize: "14px",
            }}>
              <span style={{
                width: "14px", height: "14px",
                border: `1.5px solid ${GOLD_DIM}`, borderTopColor: GOLD,
